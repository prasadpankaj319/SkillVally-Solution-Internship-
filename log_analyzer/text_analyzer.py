"""
Text Log Analyzer Module

This module handles log analysis from direct text input instead of file paths.
It provides the same analysis capabilities but works with text content directly.
"""
...
class TextLogAnalyzer:
    """
    A class for analyzing log data from direct text input.
    ...
    def __init__(self, log_content: str):
        """
        Initialize the TextLogAnalyzer with log content.
    ...
    def analyze_logs(self) -> Dict:
        """
        Analyze the log content and generate comprehensive statistics.
    ...
    def _process_line(self, line: str) -> None:
        ...
        Args:
            line (str): A single line from the log content
        """
        if not line.strip():
            return
        ...
        if level == 'ERROR':
            ...
            if message:
                # Count unique error messages (first 100 chars)
    ...
    def _generate_analysis_results(self) -> Dict:
        ...
        results = {
            'content_info': {
                'total_characters': len(self.log_content),
                'total_lines': len(self.log_content.strip().split('\n')) if self.log_content.strip() else 0
            },
    ...
    def generate_summary_report(self, analysis_results: Dict) -> str:
        ...
        report = []
        report.append("=" * 60)
        report.append("LOG ANALYSIS SUMMARY REPORT (TEXT INPUT)")
        report.append("=" * 60)
        
        # Content information
        content_info = analysis_results.get('content_info', {})
        report.append(f"\nContent Information:")
        report.append(f"  Total Characters: {content_info.get('total_characters', 0)}")
        report.append(f"  Total Lines: {content_info.get('total_lines', 0)}")
...
def get_text_input() -> str:
    """
    Get log content from user input.
    
    Returns:
        str: Log content as a string
    """
    print("Log Analyzer - Text Input Mode")
    print("=" * 50)
    print("Enter your log content (press Enter on empty line to finish):")
    print("-" * 50)
...
def analyze_text_content(log_content: str, export_json: bool = False, json_output: str = None) -> None:
    """
    Analyze log content from text and display results.
    
    Args:
        log_content (str): Log content as a string
        export_json (bool): Whether to export results to JSON
        json_output (str): Path for JSON output file
    """
    try:
        # Initialize analyzer
        analyzer = TextLogAnalyzer(log_content)
        ...
        if export_json:
            import json
            export_data = {
                'analysis_timestamp': analyzer._get_timestamp(),
                'content_information': analysis_results.get('content_info', {}),
            ...
            if json_output:
                ...
                output_path = "text_log_analysis.json"
        ...
        print(f"Error analyzing log content: {e}")
...
TextLogAnalyzer._get_timestamp = _get_timestamp
